...
export default function ProductPage() {
  ...
  return (
    <main className="flex flex-col lg:flex-row min-h-screen bg-white relative pb-20 lg:pb-0">
      
      {/* Coluna da Imagem */}
      <div className="w-full lg:w-1/2 bg-gray-50 flex items-center justify-center min-h-[50vh] lg:min-h-screen relative p-8">
      ...
      <div className="w-full lg:w-1/2 px-8 py-12 md:px-16 lg:py-24 flex flex-col justify-center">
        <div className="max-w-md mx-auto lg:mx-0 w-full">
          ...
          <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold block mb-4">
            {PRODUCT_DATA.category}
          </span>
          
          <h1 className="font-syne text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            {PRODUCT_DATA.name}
          </h1>
          
          <span className="text-2xl font-mono font-bold text-gray-900 block mb-8">
          ...
          <button 
            ...
            className="w-full bg-gray-900 text-white py-5 text-xs font-bold uppercase tracking-widest hover:bg-baza-lavender transition-all duration-300 shadow-xl hover:-translate-y-1 flex items-center justify-center gap-3 mb-12"
          >
            Adicionar à Sacola
          ...
          <div className="border-t border-gray-200">
            <div className="border-b border-gray-200">
              <button onClick={() => setActiveTab(activeTab === 'ingredients' ? '' : 'ingredients')} className="w-full py-6 flex justify-between items-center text-left">
                <span className="text-xs font-bold uppercase tracking-widest text-gray-900">Ingredientes Principais</span>
                <span className="text-xl font-light text-gray-400">{activeTab === 'ingredients' ? '-' : '+'}</span>
              </button>
              <div className={`overflow-hidden transition-all duration-300 ${activeTab === 'ingredients' ? 'max-h-40 pb-6' : 'max-h-0'}`}>
                <p className="text-sm text-gray-500 leading-relaxed">{PRODUCT_DATA.ingredients}</p>
              </div>
            </div>
            ...
            <div className="border-b border-gray-200">
              <button onClick={() => setActiveTab(activeTab === 'howToUse' ? '' : 'howToUse')} className="w-full py-6 flex justify-between items-center text-left">
                <span className="text-xs font-bold uppercase tracking-widest text-gray-900">Como Usar</span>
                <span className="text-xl font-light text-gray-400">{activeTab === 'howToUse' ? '-' : '+'}</span>
              </button>
              <div className={`overflow-hidden transition-all duration-300 ${activeTab === 'howToUse' ? 'max-h-40 pb-6' : 'max-h-0'}`}>
                <p className="text-sm text-gray-500 leading-relaxed">{PRODUCT_DATA.howToUse}</p>
              </div>
            </div>
          </div>
      ...
      <div className={`fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 p-4 shadow-[0_-4px_15px_rgba(0,0,0,0.05)] z-40 transform transition-transform duration-300 lg:hidden flex items-center justify-between gap-4 ${showSticky ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="flex flex-col">
          <span className="font-syne font-bold text-sm text-gray-900 line-clamp-1">{PRODUCT_DATA.name}</span>
          <span className="font-mono text-xs font-bold text-gray-500">R$ {PRODUCT_DATA.price.toFixed(2).replace('.', ',')}</span>
        </div>
        <button 
          onClick={handleAddToCart}
          className="bg-gray-900 text-white px-6 py-3 text-[10px] font-bold uppercase tracking-widest hover:bg-baza-lavender transition-colors whitespace-nowrap shadow-md"
        >
          Adicionar
        </button>
      </div>
