...
eventBus.on('OrderPaid', async (session) => {
  try {
    ...
    const order = await prisma.order.create({
      data: {
        totalAmount: session.amount_total, // Nota: Fica guardado em cêntimos (padrão Stripe)
        status: "PAID",
        stripeId: session.id,
        customerId: customer.id,
        // Proteção extra caso o endereço falhe
    ...
    console.log(`[EventBus] ✅ Pedido ${order.id} da Carefulbaza guardado com sucesso!`);

    // 3. Enviar E-mail de Confirmação de Compra
    const currencyCode = (session.currency || 'BRL').toUpperCase(); // toUpperCase previne erros no toLocaleString
