...
export default function LoginPage() {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      setResendMessage('O link foi reenviado! Verifique a sua caixa de entrada e SPAM.');
    } catch (err) {
      setResendMessage(err.message || 'Erro ao reenviar o link.');
    } finally {
    ...
    <main className="min-h-screen flex flex-col lg:flex-row bg-white dark:bg-gray-900 transition-colors duration-300">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 py-16 md:px-20 lg:px-32">
        <div className="w-full max-w-md mx-auto">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mb-8">Acesse sua conta para ver seus pedidos e rotinas salvas.</p>
            ...
            <div className="bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 p-4 mb-6">
              ...
              {needsVerification && (
                <div className="mt-4 flex flex-col gap-2">
                  <button onClick={handleLogin} className="text-[10px] font-bold uppercase tracking-widest text-white bg-gray-900 dark:bg-white dark:text-gray-900 py-2 px-4 hover:bg-baza-lavender dark:hover:bg-baza-mint transition-colors">
                  ...
                  <button onClick={handleResendVerification} disabled={resendLoading} className="text-[10px] font-bold uppercase tracking-widest text-red-700 dark:text-red-400 hover:text-red-900 underline text-left mt-2">
                    {resendLoading ? 'A reenviar...' : 'Reenviar E-mail de Verificação'}
                  </button>
                </div>
              )}
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              ...
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-4 py-3 text-gray-900 dark:text-white outline-none focus:border-baza-lavender transition-colors" placeholder="seu@email.com" />
            </div>
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <Link to="/recuperar-senha" className="text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-baza-mint transition-colors">Esqueceu a senha?</Link>
              </div>
              <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-4 py-3 text-gray-900 dark:text-white outline-none focus:border-baza-lavender transition-colors" placeholder="••••••••" />
            </div>
            <button type="submit" disabled={loading} className="w-full bg-gray-900 dark:bg-white text-white dark:text-gray-900 py-4 text-[10px] font-bold uppercase tracking-widest hover:bg-baza-lavender transition-colors shadow-lg flex items-center justify-center disabled:opacity-50">
              {loading ? <div className="w-5 h-5 border-2 border-white dark:border-gray-900 border-t-transparent rounded-full animate-spin"></div> : 'Acessar Conta'}
            </button>
          </form>

          <div className="mt-10 text-center border-t border-gray-100 dark:border-gray-800 pt-8">
            <p className="text-gray-500 dark:text-gray-400 text-sm">
              Ainda não tem uma conta? <Link to="/cadastro" className="font-bold text-gray-900 dark:text-white hover:text-baza-lavender transition-colors">Criar conta</Link>
            </p>
          </div>
        </div>
      </div>
