  ...
  return (
    <nav className="flex justify-between items-center py-4 sm:py-5 px-5 sm:px-8 lg:px-12 xl:px-16 border-b border-gray-100 bg-white/80 dark:bg-gray-900/90 dark:border-gray-800 backdrop-blur-md sticky top-0 z-50 transition-colors duration-300">
      ...
      <div className="flex items-center justify-end gap-4 sm:gap-5 md:gap-6 text-gray-900 dark:text-white flex-shrink-0">
        ...
        {/* 🚚 NOVO: BOTÃO DO CAMINHÃO DE RASTREIO */}
        <Link to="/rastreio" aria-label="Rastrear Pedido" title="Rastrear Pedido" className="hover:text-baza-lavender dark:hover:text-baza-mint transition-colors hidden sm:block flex-shrink-0">
        ...
        <button 
          ...
          aria-label="Cart" 
          className="text-gray-900 dark:text-white hover:text-baza-lavender dark:hover:text-baza-mint transition-colors flex items-center justify-center p-1 relative flex-shrink-0"
        >
          ...
          {cartCount > 0 && (
            <span className="absolute top-0 right-0 bg-baza-mint text-baza-lavender text-[9px] font-extrabold w-4 h-4 rounded-full flex items-center justify-center border border-white dark:border-gray-900 shadow-sm transform translate-x-1/4 -translate-y-1/4">
              {cartCount}
            </span>
          )}
        </button>
